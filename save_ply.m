function save_ply(filename,points,faces,mode)

% SAVE_PLY - write data to PLY file.
%
%   SAVE_PLY(VFILENAME,POINTS,FACES,MODE)
%
...
if size(points,2)~=3
    error('points does not have correct format.');
end
...
if size(faces,2)~=3
    error('faces does not have correct format.');
end

% make a cube
clear Data;
Data.vertex.x = points(:,1);
Data.vertex.y = points(:,2);
Data.vertex.z = points(:,3);
Data.face.vertex_indices = {};
for i=1:size(faces,1)
    Data.face.vertex_indices{end+1} = faces(i,:)-1;
end
